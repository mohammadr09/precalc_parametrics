...
def ellipse_demo_display(scene):
    ...
    ground = Line(LEFT * 7.5, RIGHT * 7.5, color=TEAL, stroke_width=2).set_y(-2.2)
 
    scene.play(
        FadeOut(ellipse, a_line, b_line, a_label, b_label,
                ellipse_eqs, domain, dot),
        FadeIn(ground),
        run_time=1.2,
    )
 
    return ground   # hand off to cycloid so it doesn't redraw it
 
 
def cycloid_demo_display(scene, ground):
    r = 0.75 
    ground_y = -2.2
    ...
    domain = MathTex(r"t \in [0, 2\pi]").scale(0.8).next_to(param_eqs, DOWN, buff=0.3)
    ...
    spoke = Line(ORIGIN, DOWN * r, color=WHITE, stroke_width=1.5)
    center_dot = Dot(ORIGIN, color=WHITE, radius=0.05)
    rolling_base = VGroup(circle, spoke, center_dot)

    rolling = always_redraw(
        lambda: rolling_base.copy()
        .rotate(-tracker.get_value(), about_point=ORIGIN)
        .move_to(get_rolling_center(tracker.get_value()))
    )
...
class ParametricPresentation(Scene):
    def construct(self):
        ...
        ground = ellipse_demo_display(self)  # returns the ground line
        cycloid_demo_display(self, ground)